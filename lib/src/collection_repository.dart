...
class FirestoreCollectionRepository<T extends JsonModel>
  ...
  ///
  /// Example: `await repo.batchAdd([{'n': 1}, {'n': 2}]);`
  Future<void> batchAdd(List<Map<String, dynamic>> items) async {
    if (items.isEmpty) return;
    final col = _colOrThrow();
    for (var i = 0; i < items.length; i += _batchLimit) {
      final chunk = items.sublist(
        i,
        (i + _batchLimit).clamp(0, items.length),
      );
  ...
  ///
  /// Each model's `id` determines the document path. Existing documents
  /// are merged via [SetOptions(merge: true)].
  ///
  /// Example: `await repo.batchSet([model1, model2]);`
  Future<void> batchSet(List<T> models) async {
    if (models.isEmpty) return;
    final col = _colOrThrow();
    for (var i = 0; i < models.length; i += _batchLimit) {
      final chunk = models.sublist(
        i,
        (i + _batchLimit).clamp(0, models.length),
      );
      final batch = _fs.batch();
  ...
  /// to update. Only the specified fields are modified.
  ///
  /// Example: `await repo.batchPatch([(id: 'a', data: {'n': 1}), ...]);`
  Future<void> batchPatch(List<Patch> patches) async {
    if (patches.isEmpty) return;
    final col = _colOrThrow();
    for (var i = 0; i < patches.length; i += _batchLimit) {
      final chunk = patches.sublist(
        i,
        (i + _batchLimit).clamp(0, patches.length),
      );
      final batch = _fs.batch();
      for (final p in chunk) {
        batch.update(col.doc(p.id), p.data);
      }
      await batch.commit();
    }
  }
  ...
  ///
  /// Example: `await repo.batchDelete(['id1', 'id2', 'id3']);`
  Future<void> batchDelete(List<String> docIds) async {
    if (docIds.isEmpty) return;
    final col = _colOrThrow();
    for (var i = 0; i < docIds.length; i += _batchLimit) {
      final chunk = docIds.sublist(
        i,
        (i + _batchLimit).clamp(0, docIds.length),
      );
      final batch = _fs.batch();
